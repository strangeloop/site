  ...
  module Admin
    class ProposalsController < Refinery::AdminController
      ...
      before_filter(:only =>[:rate, :add_comment]) do
        redirect_to(root_path) unless current_refinery_user.has_role? :reviewer
      end

      before_filter(:only => [:export]) do
        redirect_to(root_path) unless current_refinery_user.has_role? :organizer
      end
      ...
      def rate
        @proposal = proposal.tap do |proposal|
          proposal.rate(params[:stars], current_refinery_user, params[:dimension])
      ...
      def add_comment
        proposal.comments.create(:comment => params[:comment], :user => current_refinery_user)
        #the reload is needed in case the status changed,
        #I wish this could be more elegant
        proposal.reload
